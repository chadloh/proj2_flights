...
fare_pair <-
  ...
  summarise(nsmiles = mean(nsmiles), passengers_sum = sum(passengers), fare_mean = weighted.mean(fare,passengers)) %>%
# join with airports_wiki data for longitude and latitude
...
flights_pair2 %>%
  ...
  rename(city2 = City) %>%
  left_join(fare_pair[1:5], by = c("city1" = "city1", "city2" = "city2")) %>%
  left_join(fare_pair[1:5], by = c("city1" = "city2", "city2" = "city1")) %>%
  mutate(nsmiles = coalesce(nsmiles.x, nsmiles.y)) %>% 
  mutate(passengers_sum = coalesce(passengers_sum.x, passengers_sum.y)) %>% 
  mutate(fare_mean = coalesce(fare_mean.x, fare_mean.y)) %>% 
  mutate(ppm = fare_mean/nsmiles) %>% 
  select(-nsmiles.x, -nsmiles.y, 
         -passengers_sum.x, -passengers_sum.y, 
         -fare_mean.x, -fare_mean.y) %>%
  na.omit()

for(i in 1:nrow(ppm_df)){
  ppm_df$Hubs[i] = grepl(ppm_df$port1[i], ppm_df$Hubs[i]) | grepl(ppm_df$port2[i], ppm_df$Hubs[i]) 
}

ppm_df <-
  ppm_df %>%
  mutate(Hubs = ifelse(Hubs,"Hub flight", "Not a hub flight"))
...
ggplot(data = ppm_df, aes(x = dist, y = fare_mean, color = carrier_name)) +
...
ggplot(ppm_df, aes(x=Hubs, y=ppm)) + 
  ...
  facet_wrap(.~carrier_name, ncol = 5) +
...
```{r}
write.csv(flights_pair,"flights_pair.csv", row.names = FALSE)
